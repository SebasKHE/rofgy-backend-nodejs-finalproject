...
const SECRET_KEY = 'cpktnwt';

mongoose.set('strictQuery', false);

const uri =  "mongodb://root:CM6DmB6YsBY98d69ByRiNC0q@172.21.48.178:27017";
...
app.put('/posts/:postId', authenticateJWT, (req, res) => {
    ...
    const postIndex = posts.findIndex((post) => postId == post.postId && post.userId === req.user.userId);

    if (postIndex === -1) return res.status(400).json({message: 'Post not found'});

    posts[postIndex].text = text;

    res.json({message: 'Post updated succesfully', updatedPost: posts[postIndex]});
});
